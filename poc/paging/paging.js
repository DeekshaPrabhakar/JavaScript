let citiesModule = (function () {
    ...
    let controller = {
        ...
        loadAllCities: function() {
            ...
            let cPromise = fetch(endpoint)
                .then(blob => blob.json())
                .then(data => {
                    model.allCities.push(...data)
                    render();
                });
        }, 
        setCities: function(data) {
            model.allCities = data;
            view.render();
        },
        getPagedData: function(search) {
            if (search.length == 0) {
                model.cities = model.allCities;
            }
            else {
                model.cities = this.findMatches(search, model.allCities);
            }
    
            let startIndex = (this.currentPage) * this.pageSize;
            let endIndex = startIndex + this.pageSize;
            return model.cities.slice(startIndex, endIndex);
        },
    ...
    let view = {
        ...
        cacheDOM: function(){
            this.module = document.getElementById("pageContent");
            this.citiesPH = this.module.querySelector("#cities");
            this.template = this.module.querySelector("#cityRowTemplate").innerHTML;
            this.searchInput = this.module.querySelector('.search');
        },
        bindEvents: function(){
            this.searchInput.addEventListener('change', this.render.bind(this));
            this.searchInput.addEventListener('keyup', this.render.bind(this));        
        },
        render: function() {
            let rowsHTML = "";
            let data = controller.getPagedData(this.searchInput.value);
    
            for (let city of data) {
                rowsHTML += this.template.replace(/{{rank}}/g, city.rank)
                    .replace(/{{city}}/g, city.city)
                    .replace(/{{state}}/g, city.state);
            }
    
            this.citiesPH.innerHTML = rowsHTML;
        }
    }     
    ...
    //public interface
    return {
        setCities: controller.setCities
    }
